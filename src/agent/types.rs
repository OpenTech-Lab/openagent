...
use super::agentic_loop::LoopTrace;
...
/// State machine states for the Planner-Worker-Reflector pattern
#[derive(Debug, Clone)]
pub enum AgentState {
    /// Planning phase - LLM is creating an execution plan
    Planning {
        messages: Vec<Message>,
        attempt: u32,
    },
    /// Executing phase - Worker is executing the plan steps
    Executing {
        plan: ExecutionPlan,
        step_index: usize,
    ...
    /// Reflecting phase - LLM is reviewing execution results
    Reflecting {
        plan: ExecutionPlan,
        results: Vec<StepResult>,
        messages: Vec<Message>,
    },
    /// Complete - Final answer achieved
    Complete {
        response: String,
        trace: LoopTrace,
        messages: Vec<Message>,
...
pub struct PlanStep {
    ...
    /// Dependencies on other steps (step indices)
    #[serde(default)]
...
impl StepResult {
    ...
    pub fn success(step_index: usize, content: String, duration_ms: u64) -> Self {
        Self {
            step_index,
            success: true,
    ...
    pub fn failure(step_index: usize, error: String, duration_ms: u64) -> Self {
        Self {
            step_index,
            success: false,
            content: None,
